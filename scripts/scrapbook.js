...
hexo.extend.filter.register('after_render:html', function (html, data) {
  const isPage = ['projects/index.html', 'about/index.html'].includes(data.path);
  if (!isPage && (!/^(blog|categories|tags|archives)\//.test(data.path || '') || !html.includes('class="l_body index"'))) return html;
  ...
  return html
    ...
    .replace('<body>', `<body class="scrapbook-list${isPage ? ' scrapbook-page' : ''}"><a class="skip-link" href="#main">跳到主要内容</a>${navigation}${renderSearchPanel()}`)
